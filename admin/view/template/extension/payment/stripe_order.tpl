...
<table class="table table-striped table-bordered">
  ...
  <tr style="color: green">
    <td>Balance</td>
    <td>
        <?php echo number_format(($charge['amount'] - $charge['amount_refunded']) / 100, 2); ?> <?php echo  $charge['currency'] ?>
    </td>
  </tr>
  ...
  <tr>
      ...
      <td>
          <table class="table table-striped table-bordered" id="stripe_refunds_table">
                  ...
                  <tbody>
                    <?php foreach ($charge['refunds']['data'] as $refund) { //  prd($refund);
                        ?>
                    <tr>
                      <td class="text-left"><?php echo number_format($refund['amount'] / 100,2); ?> <?php echo $refund['currency']; ?></td>
  ...
  $("#button-refund").click(function() {
        if (confirm('<?php echo $text_confirm_refund; ?>')) {
            $.ajax({
                ...
                url: '?route=extension/payment/stripe/refund&token=<?php echo $token; ?>',
                beforeSend: function() {
                ...
                success: function(data) {
                    if (data.error == false) {
                      ...
                      $table.prepend('<tr><td class="text-left">'+$('#input-refund-amount').val()+'</td><td>'+ new Date().today() + " @ " + new Date().timeNow()+'</td><td>--</td></tr>');
                    }
                    if (data.error == true) {
                        alert(data.msg);
                    }
